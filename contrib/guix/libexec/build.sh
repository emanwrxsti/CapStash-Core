#!/usr/bin/env bash
# Copyright (c) 2019-2022 The CapStash Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} CAPSTASH_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our CapStash Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built CapStash Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example CapStash.conf file. if contrib/devtools/gen-CapStash-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/CapStash.conf" "${DISTNAME}/"
