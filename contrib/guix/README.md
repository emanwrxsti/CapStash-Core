# Bootstrappable CapStash Core Builds

This directory contains the files necessary to perform bootstrappable CapStash
Core builds.
...
To build CapStash Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      CapStash Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/CapStash-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/CapStash-core/guix.sigs).
