...
""" Example logging CapStash Core mempool events using the mempool:added,
...
def main(CapStashd_path):
    CapStashd_with_usdts = USDT(path=str(CapStashd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    CapStashd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    CapStashd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    CapStashd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    CapStashd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[CapStashd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/CapStashd")
        exit(1)
