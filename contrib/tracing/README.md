...
Tracing (USDT) support for CapStash Core on Linux using. For more information on
USDT support in CapStash Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `CapStashd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `CapStashd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[CapStash Core USDT documentation] on how to list available tracepoints in your
`CapStashd` binary.

[CapStash Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/CapStashd
```
...
CapStash P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/CapStashd
```
...
In a different terminal, starting CapStash Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/CapStashd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/CapStashd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/CapStashd
```
