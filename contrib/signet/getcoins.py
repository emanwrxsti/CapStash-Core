#!/usr/bin/env python3
# Copyright (c) 2020-2022 The CapStash Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing CapStash-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='CapStash-cli', help='CapStash-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='CapStash address to which the faucet should send')
...
parser.add_argument('CapStash_cli_args', nargs='*', help='Arguments to pass on to CapStash-cli (default: -signet)')

args = parser.parse_args()

if args.CapStash_cli_args == []:
    args.CapStash_cli_args = ['-signet']


def CapStash_cli(rpc_command_and_params):
    argv = [args.cmd] + args.CapStash_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = CapStash_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = CapStash_cli(['getnewaddress', 'faucet', 'bech32'])
