...
"""Script for verifying CapStash Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from CapStashcore.org and CapStash.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/CapStash-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://CapStashcore.org"
HOST2 = "https://CapStash.org"
VERSIONPREFIX = "CapStash-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the CapStash community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"CapStash_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by CapStashcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the CapStash release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes CapStash.org lags behind CapStashcore.org.)')
    )
