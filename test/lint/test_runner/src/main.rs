// Copyright (c) The CapStash Core developers
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./src/config/CapStash-config.h.in";
    let include_directive = "#include <config/CapStash-config.h>";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args([
                    // These are exceptions which don't use CapStash-config.h, rather the Makefile.am adds
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the CapStash-config.h header. However, they are not
...
even though CapStash-config.h indicates that a faster feature is available and should be used.
...
git grep --perl-regexp '{}' -- file_name
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the CapStash-config.h header. However, they are including
