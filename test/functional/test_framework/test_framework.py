#!/usr/bin/env python3
# Copyright (c) 2014-2022 The CapStash Core developers
...
TMPDIR_PREFIX = "CapStash_func_test_"
...
class CapStashTestMetaClass(type):
    """Metaclass for CapStashTestFramework.

    Ensures that any attempt to register a subclass of `CapStashTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'CapStashTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("CapStashTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("CapStashTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class CapStashTestFramework(metaclass=CapStashTestMetaClass):
    """Base class for a CapStash test script.

    Individual CapStash test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave CapStashds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop CapStashds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use CapStash-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "CapStashd": ("CapStashd", "CAPSTASHD"),
            "CapStash-cli": ("CapStashcli", "CAPSTASHCLI"),
            "CapStash-util": ("CapStashutil", "CAPSTASHUTIL"),
            "CapStash-wallet": ("CapStashwallet", "CAPSTASHWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: CapStashds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("CapStashRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'CapStashd', self.options.CapStashd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'CapStash-cli', self.options.CapStashcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a CapStashd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple CapStashd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the CapStashd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as CapStashd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    CapStashd=self.options.CapStashd,
                    CapStash_cli=self.options.CapStashcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in CapStash.conf
    ...
    def skip_if_no_CapStashd_tracepoints(self):
        """Skip the running test if CapStashd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("CapStashd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_CapStashd_zmq(self):
        """Skip the running test if CapStashd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("CapStashd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if CapStash-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("CapStash-wallet has not been compiled")

    def skip_if_no_CapStash_util(self):
        """Skip the running test if CapStash-util has not been compiled."""
        if not self.is_CapStash_util_compiled():
            raise SkipTest("CapStash-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if CapStash-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("CapStash-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether CapStash-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether CapStash-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_CapStash_util_compiled(self):
        """Checks whether CapStash-util was compiled."""
        return self.config["components"].getboolean("ENABLE_CAPSTASH_UTIL")
