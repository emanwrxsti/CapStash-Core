...
double GetDifficulty(const CBlockIndex& blockindex)
{
    ...
    double dDiff =
        (double)0x0001fffe / (double)(blockindex.nBits & 0x00ffffff);
...
UniValue blockheaderToJSON(const CBlockIndex& tip, const CBlockIndex& blockindex)
{
    ...
    // Consensus PoW hash (Whirlpool-based) computed from the header fields
    const CBlockHeader hdr = blockindex.GetBlockHeader();
    result.pushKV("powhash", hdr.GetPoWHash().GetHex());
...
static RPCHelpMan getblockhashes()
{
    return RPCHelpMan{"getblockhashes",
                "\nReturns both the standard block hash (header ID) and the proof-of-work hash for a block.\n"
                "The argument may be a 64-hex block hash, or a decimal height (as a string).\n",
                {
                    {"block", RPCArg::Type::STR, RPCArg::Optional::NO, "Block hash (64 hex) or block height (decimal string)"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::NUM,     "height",   "The block height"},
                        {RPCResult::Type::STR_HEX, "blockhash","The standard block hash / header ID (sha256d)"},
                        {RPCResult::Type::STR_HEX, "powhash",  "The proof-of-work hash (Whirlpool-derived)"},
                ...
                RPCExamples{
                    HelpExampleCli("getblockhashes", "\"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09\"") +
                    HelpExampleCli("getblockhashes", "\"1000\"") +
                    HelpExampleRpc("getblockhashes", "\"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09\"")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    ...
    const std::string arg = request.params[0].get_str();
    ...
    try {
        size_t idx{0};
        height = std::stoi(arg, &idx, 10);
        height_ok = (idx == arg.size());
    } catch (...) {
        height_ok = false;
    }
    ...
    if (height_ok) {
        ...
        // Block hash
        if (!(arg.size() == 64 && IsHex(arg))) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Argument must be a 64-hex block hash or a decimal height string");
        }
        const uint256 hash = ParseHashV(request.params[0], "block");
    ...
    out.pushKV("height", pindex->nHeight);
    out.pushKV("blockhash", hdr.GetHash().GetHex());
    out.pushKV("powhash", hdr.GetPoWHash().GetHex());
    out.pushKV("previousblockhash", hdr.hashPrevBlock.GetHex());
    out.pushKV("merkleroot", hdr.hashMerkleRoot.GetHex());
    out.pushKV("time", (int64_t)hdr.nTime);
    out.pushKV("bits", strprintf("%08x", hdr.nBits));
    out.pushKV("nonce", (uint64_t)hdr.nNonce);
    return out;
},
    };
}
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "powhash", "the proof-of-work hash for this header (hex-encoded)"},
        ...
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::OBJ, "prevout", "(Only if undo information is available)",
            {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/true, "The CapStash address (only if a well-defined address exists)"},
...
static RPCHelpMan getblock()
{
    return RPCHelpMan{"getblock",
                ...
                {
                    {RPCResult::Type::STR_HEX, "hash", "the block hash (same as provided)"},
                    {RPCResult::Type::STR_HEX, "powhash", "the proof-of-work hash for this block header (hex-encoded)"},
                    {RPCResult::Type::NUM, "confirmations", "The number of confirmations, or -1 if the block is not on the main chain"},
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (CapStash-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
    ...
    const fs::path& temppath)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/CapStash/CapStash/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
...
static RPCHelpMan loadtxoutset()
{
    return RPCHelpMan{
        ...
        "The result is a usable CapStashd instance that is current with the network tip in a "
        ...
        "document (<https://github.com/CapStash/CapStash/blob/master/doc/design/assumeutxo.md>).",
        {
            {"path",
                RPCArg::Type::STR,
                RPCArg::Optional::NO,
