...
#include <qt/CapStashaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CapStash address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CapStashAddressEntryValidator(parent));
    widget->setCheckValidator(new CapStashAddressCheckValidator(parent));
}
...
bool parseCapStashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no CapStash: URI
    if(!uri.isValid() || uri.scheme() != QString("CapStash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!CapStashUnits::parse(CapStashUnit::Cap, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseCapStashURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCapStashURI(uriInstance, out);
}

QString formatCapStashURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("CapStash:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CapStashUnits::format(CapStashUnit::Cap, info.amount, false, CapStashUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCapStashConf()
{
    ...
    /* Open CapStash.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CapStash.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CapStash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("CapStash (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for CapStash*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "CapStash.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("CapStash-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a CapStash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=CapStash\n";
        else
            optionFile << strprintf("Name=CapStash (%s)\n", ChainTypeToString(chain));
