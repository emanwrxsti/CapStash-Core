      ...
      platformStyle(_platformStyle)
{
    ...
    minerPage = new MinerPage(this);
    minerPage->setClientModel(clientModel);
    minerPage->setWalletModel(walletModel);
    addWidget(minerPage);
    ...
    m_export_button->setToolTip(tr("Export the data in the current tab to a file"));
    if (platformStyle->getImagesOnButtons()) {
    m_export_button->setIcon(platformStyle->SingleColorIcon(":/icons/export"));
    }
    hbox_buttons->addStretch();
    hbox_buttons->addWidget(m_export_button);
    ...
    connect(m_export_button, &QPushButton::clicked, transactionView, &TransactionView::exportClicked);
...
void WalletView::reapplyTheme()
{
    if (m_export_button && platformStyle && platformStyle->getImagesOnButtons()) {
    m_export_button->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/export")));
    m_export_button->update();
    }

    if (sendCoinsPage) {
        sendCoinsPage->refreshTheme();
        sendCoinsPage->update();
    }

    if (receiveCoinsPage) {
        receiveCoinsPage->refreshTheme();
        receiveCoinsPage->update();
    }

    if (minerPage) {
        minerPage->refreshTheme();
        minerPage->update();
    }

    if (explorerPage) {
        explorerPage->reapplyTheme();
        explorerPage->update();
    }
