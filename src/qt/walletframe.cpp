...
void WalletFrame::reapplyTheme()
{
    for (auto i = mapWalletViews.constBegin(); i != mapWalletViews.constEnd(); ++i) {
        if (i.value()) {
            i.value()->reapplyTheme();
        }
    }
}
...
{
    if (currentWalletView()) currentWalletView()->gotoMinerPage();
}

void WalletFrame::gotoExplorerPage()
{
    if (currentWalletView()) currentWalletView()->gotoExplorerPage();
}
