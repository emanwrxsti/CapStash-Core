...
PipBoyTheme::Palette CurrentThemePalette()
{
    return PipBoyTheme::GetPalette(PipBoyTheme::CurrentMode());
}
      ...
      bestHeaderDate(QDateTime())
{
    ...
    const auto p = CurrentThemePalette();

    QWidget* bg = findChild<QWidget*>("bgWidget");
    QWidget* card = findChild<QWidget*>("contentWidget");
    ...
    if (bg) bg->setStyleSheet(QString());
    if (card) card->setStyleSheet(QString());
    ...
    if (card) {
        card->setStyleSheet(QString(
            ...
            "  background: %8;"
            "}"
        ).arg(CssColor(p.panel),          // %1
              CssColor(p.modal_border),   // %2
              CssColor(p.text),           // %3
              CssColor(p.border),         // %4
              CssColor(p.selection_text), // %5
              CssColor(p.selection_bg),   // %6
              CssColor(p.accent),         // %7
              CssColor(p.pressed)));      // %8
    }
...
void ModalOverlay::UpdateHeaderSyncLabel()
{
    ...
    ui->numberOfBlocksLeft->setText(
        tr("Unknown. Syncing Headers (%1, %2%)…")
            ...
            .arg(QString::number(100.0 / (bestHeaderHeight + est_headers_left) * bestHeaderHeight, 'f', 1)));
}
...
void ModalOverlay::UpdateHeaderPresyncLabel(int height, const QDateTime& blockDate)
{
    ...
    ui->numberOfBlocksLeft->setText(
        tr("Unknown. Pre-syncing Headers (%1, %2%)…")
            .arg(height)
            .arg(QString::number(100.0 / (height + est_headers_left) * height, 'f', 1)));
}
