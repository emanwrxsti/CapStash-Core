...
#include <qt/CapStash.h>
// src/qt/capstashgui.cpp
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<CapStashUnit>("CapStashUnit");
#else
    qRegisterMetaType<CapStashUnit>("CapStashUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. CapStash_de.qm (shortcut "de" needs to be defined in CapStash.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. CapStash_de_DE.qm (shortcut "de_DE" needs to be defined in CapStash.qrc)
...
static const char* qt_argv = "CapStash-qt";

CapStashApplication::CapStashApplication()
...
void CapStashApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the CapStashApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", CapStashGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void CapStashApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool CapStashApplication::createOptionsModel(bool resetSettings)
{
...
void CapStashApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new CapStashGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &CapStashGUI::quitRequested, this, &CapStashApplication::requestShutdown);
...
void CapStashApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void CapStashApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool CapStashApplication::baseInitialize()
{
    return node().baseInitialize();
}

void CapStashApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &CapStashApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &CapStashApplication::handleRunawayException);
    connect(this, &CapStashApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &CapStashApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void CapStashApplication::parameterSetup()
{
...
void CapStashApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void CapStashApplication::requestInitialize()
{
...
void CapStashApplication::requestShutdown()
{
...
void CapStashApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // CapStash: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &CapStashGUI::handlePaymentRequest);
            connect(window, &CapStashGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void CapStashApplication::handleRunawayException(const QString &message)
{
...
void CapStashApplication::handleNonFatalException(const QString& message)
{
...
WId CapStashApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool CapStashApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", CapStashGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(CapStash);
    Q_INIT_RESOURCE(CapStash_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(CAPSTASH_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see CapStash-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see CapStash-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    QSettings settings;
    const QString phosphor_theme = settings.value("phosphorTheme", "green").toString();

    if (phosphor_theme == "amber") {
        PipBoyTheme::SetMode(PipBoyTheme::Mode::Amber);
    } else {
        PipBoyTheme::SetMode(PipBoyTheme::Mode::Green);
    }
    ...
    /// 6-7. Parse CapStash.conf, determine network, switch to network specific
    ...
    // CapStash: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
