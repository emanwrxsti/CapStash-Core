...
/** Call node.updateRwSetting() with CapStash 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because CapStash 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/CapStash/CapStash/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because CapStash 22.x will not try to read these.
...
//! Convert enabled/size values to CapStash -prune setting.
...
//! Get pruning enabled value to show in GUI from CapStash -prune setting.
...
//! Get pruning size value to show in GUI from CapStash -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCapStashUnit")) {
        settings.setValue("DisplayCapStashUnit", QVariant::fromValue(CapStashUnit::Cap));
    }
    QVariant unit = settings.value("DisplayCapStashUnit");
    if (unit.canConvert<CapStashUnit>()) {
        m_display_CapStash_unit = unit.value<CapStashUnit>();
    } else {
        m_display_CapStash_unit = CapStashUnit::Cap;
        settings.setValue("DisplayCapStashUnit", QVariant::fromValue(m_display_CapStash_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if CapStash.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_CapStash_unit);
...
{
    if (new_unit.isNull() || new_unit.value<CapStashUnit>() == m_display_CapStash_unit) return;
    m_display_CapStash_unit = new_unit.value<CapStashUnit>();
    QSettings settings;
    settings.setValue("DisplayCapStashUnit", QVariant::fromValue(m_display_CapStash_unit));
    Q_EMIT displayUnitChanged(m_display_CapStash_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/CapStash/CapStash/pull/8273
    ...
    // (https://github.com/CapStash-core/gui/issues/567).
