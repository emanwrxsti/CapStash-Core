...
void ReceiveCoinsDialog::refreshTheme()
{
    if (!platformStyle) return;

    ui->clearButton->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/remove")));
    ui->receiveButton->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/receiving_addresses")));
    ui->showRequestButton->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/eye")));
    ui->removeRequestButton->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/remove")));
...
void ReceiveCoinsDialog::copyURI()
{
    ...
    const RecentRequestsTableModel * const submodel = model->getRecentRequestsTableModel();
    const QString uri = GUIUtil::formatCapStashURI(submodel->entry(sel.row()).recipient);
