...
We aim to cut a regular release every 3-4 months, approximately twice as frequent as major CapStash Core releases. Every second release should be published one month before the feature freeze of the next major CapStash Core release, allowing sufficient time to update the library in Core.
...
2. After the PR is merged, tag the commit and push it:
   ...
   git push git@github.com:CapStash-core/secp256k1.git v$MAJOR.$MINOR.$PATCH
   ```
...
1. If there's no maintenance branch `$MAJOR.$MINOR`, create one:
   ...
   git push git@github.com:CapStash-core/secp256k1.git $MAJOR.$MINOR
   ```
