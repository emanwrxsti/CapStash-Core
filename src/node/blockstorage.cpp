    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // CapStash: block ID hash is SHA256d for indexing/explorer compatibility,
                // but PoW uses Yespower. Reconstruct the header and validate PoW using
                // the header overload so CheckProofOfWork() uses GetPoWHash().
                CBlockHeader header;
                ...
                if (!CheckProofOfWork(header, consensusParams)) {
