...
const char * const CAPSTASH_CONF_FILENAME = "CapStash.conf";
const char * const CAPSTASH_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //CapStash-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", CAPSTASH_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\CapStash
    // macOS: ~/Library/Application Support/CapStash
    // Unix-like: ~/.CapStash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CapStash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/CapStash";
#else
    // Unix-like
    return pathRet / ".CapStash";
#endif
#endif
}
