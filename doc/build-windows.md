...
Below are some notes on how to build CapStash Core for Windows.

The options known to work for building CapStash Core on Windows are:
...
Note that for WSL the CapStash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/CapStash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\CapStash`, for example:

    make install DESTDIR=/mnt/c/workspace/CapStash
