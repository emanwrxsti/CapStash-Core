...
This tutorial will describe how to use two instances of CapStash Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed CapStash Transactions](https://github.com/CapStash/CapStash/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
## Overview
In this tutorial we have two hosts, both running CapStash v25.0
...
```sh
[offline]$ ./src/CapStash-cli -signet -named createwallet \
...
```sh
[offline]$ ./src/CapStash-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/CapStash-cli -signet -named createwallet \
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./src/CapStash-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/CapStash-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/CapStash-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/CapStash-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/CapStash-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/CapStash-cli -signet -rpcwallet="watch_only_wallet" listtransactions
