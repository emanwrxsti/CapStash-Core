...
CapStash Core version 0.20.0 is now available from:

  <https://CapStashcore.org/bin/CapStash-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `CAPSTASH_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from CapStash Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since CapStash Core version 0.18.0.
...
- Testing or debugging of implementations of the CapStash P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of CapStash Core. CapStash Core logs debug messages
...
- OpenSSL is no longer used by CapStash Core.  (#17265)

- BIP70 support has been fully removed from CapStash Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/CapStash/CapStash/issues/16599),
  [PR #16702](https://github.com/CapStash/CapStash/pull/16702), and the
  `CapStashd help` for more information.  This option is experimental and
...
- All custom settings configured when CapStash Core starts are now
...
- The "Start CapStash Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://CapStashcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple CapStash Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed CapStash Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/CapStash-core/HWI) for
  signing.  Future versions of CapStash Core should support a GUI option
...
- CapStash Core's automatically-generated source code documentation is
  now available at https://doxygen.CapStashcore.org.  (#17596)
...
- The `CapStash-cli` utility used with the `-getinfo` parameter now
...
- The `CapStash-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `CapStashstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_CapStash-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBCAPSTASH_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_CapStash` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBCAPSTASH_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for CapStash-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate CapStash schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest CapStash.conf (sdaftuar)
...
- #18022 Fix appveyor `test_CapStash` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (CapStash-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone CapStash-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about CapStash Core files (hebasto)
...
- #17119 Fix broken CapStash-cli examples (andrewtoth)
...
- #17503 Remove bitness from CapStash-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.CapStashcore.org (laanwj)
...
- #17648 Rename wallet-tool references to CapStash-wallet (hel-o)
...
- #18341 Replace remaining literal Cap with `CURRENCY_UNIT` (domob1812)
...
- #16115 On CapStashd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from CapStash-cli -getinfo (malevolent)
