# CJDNS support in CapStash Core

It is possible to run CapStash Core over CJDNS, an encrypted IPv6 network that
...
done outside of CapStash Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the CapStash network and individual nodes.
...
before it will work with your CapStash Core node. This is described in steps
...
## Run CapStash Core with CJDNS

Once you are connected to the CJDNS network, the following CapStash Core
...
When enabled, this option tells CapStash Core that it is running in an
...
IPv6 local network. This helps CapStash Core perform better address management:
...
CJDNS support was added to CapStash Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `CapStash-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/CapStash/CapStash/blob/master/doc/tor.md) for
details.

## CJDNS-related information in CapStash Core

There are several ways to see your CJDNS address in CapStash Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `CapStash-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `CapStash-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `CapStash-cli help getnodeaddresses` for details.
