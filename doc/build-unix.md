UNIX BUILD NOTES
====================
Some notes on how to build CapStash Core in Unix.
...
memory available when compiling CapStash Core. On systems with less, gcc can be
...
To build CapStash Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build CapStash-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a CapStash-qt executable will be
built by default.
...
...
to: /path/to/CapStash/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/CapStash/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, CapStash Core can
