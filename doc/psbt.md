# PSBT Howto for CapStash Core

Since CapStash Core 0.17, an RPC interface exists for Partially Signed CapStash
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/CapStash/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for CapStash transactions that are not fully signed
...
#### Multisig with multiple CapStash Core instances
...
CapStash Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `CapStash-cli` is used.
...
Later, when *V* Cap has been received on *Amulti*, and Bob and Carol want to
