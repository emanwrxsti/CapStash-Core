# User-space, Statically Defined Tracing (USDT) for CapStash Core

CapStash Core includes statically defined tracepoints to allow for more
...
**Note**: CapStash Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to CapStash Core
...
Multiple tools can list the available tracepoints in a `CapStashd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in CapStash Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/CapStashd
stap net        outbound_message 0x0000000000107c05 /src/CapStashd
stap validation block_connected  0x00000000002fb10c /src/CapStashd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in CapStash Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/CapStashd | grep NT_STAPSDT -A 4 -B 2
